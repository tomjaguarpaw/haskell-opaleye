...
import           Opaleye.Internal.Column (Column(Column), unsafeCase_, Nullability(..)
...
restrict :: QueryArr (Column 'NonNullable T.PGBool) ()
...
\"point free\" style.-}
keepWhen :: (a -> Column 'NonNullable T.PGBool) -> QueryArr a a
...
infix 4 .==
(.==) :: Column n a -> Column n a -> Column 'NonNullable T.PGBool
(.==) = C.binOp (HPQ.:==)
...
-- `.==` in a future version.
(.===) :: D.Default O.EqPP columns columns => columns -> columns -> Column 'NonNullable T.PGBool
(.===) = (O..==)
...
infix 4 .>
(.>) :: Ord.PGOrd a => Column n a -> Column n a -> Column 'NonNullable T.PGBool
(.>) = unsafeGt
...
case_ :: [(Column 'NonNullable T.PGBool, Column n a)] -> Column n a -> Column n a
case_ = unsafeCase_

-- | Monomorphic if\/then\/else.
--
-- This may be replaced by 'ifThenElseMany' in a future version.
ifThenElse :: Column 'NonNullable T.PGBool -> Column n a -> Column n a -> Column n a
ifThenElse = unsafeIfThenElse

-- | Polymorphic if\/then\/else.
ifThenElseMany :: D.Default O.IfPP columns columns
               => Column 'NonNullable T.PGBool
               -> columns
               -> columns
               -> columns
...
-- | Boolean or
(.||) :: Column 'NonNullable T.PGBool -> Column 'NonNullable T.PGBool -> Column 'NonNullable T.PGBool
(.||) = C.binOp HPQ.OpOr
...
-- | Boolean not
not :: Column 'NonNullable T.PGBool -> Column 'NonNullable T.PGBool
not = C.unOp HPQ.OpNot

-- | True when any element of the container is true
ors :: F.Foldable f => f (Column 'NonNullable T.PGBool) -> Column 'NonNullable T.PGBool
ors = F.foldl' (.||) (T.pgBool False)

-- * Text operators

-- | Concatenate 'Column' 'NonNullable' 'T.PGText'
(.++) :: Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText
(.++) = C.binOp (HPQ.:||)

-- | To lowercase
lower :: Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText
lower = C.unOp HPQ.OpLower

-- | To uppercase
upper :: Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText
upper = C.unOp HPQ.OpUpper

-- | Postgres @LIKE@ operator
like :: Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText -> Column 'NonNullable T.PGBool
like = C.binOp HPQ.OpLike

-- | Postgres @ILIKE@ operator
ilike :: Column 'NonNullable T.PGText -> Column 'NonNullable T.PGText -> Column 'NonNullable T.PGBool
ilike = C.binOp HPQ.OpILike

charLength :: C.PGString a => Column n a -> Column 'NonNullable Int
...
in_ :: (Functor f, F.Foldable f) => f (Column n a) -> Column n a -> Column 'NonNullable T.PGBool
...
inQuery :: D.Default O.EqPP columns columns
        => columns -> Query columns -> Query (Column 'NonNullable T.PGBool)
inQuery c q = qj'
  where -- Remove every row that isn't equal to c
        ...
        qj :: Query (Column 'NonNullable T.PGInt4, Column 'Nullable T.PGInt4)
        qj = Join.leftJoin (A.arr (const 1))
                           (Distinct.distinct q')
                           (uncurry (.==))

        -- Check whether it is 'NULL'
        qj' :: Query (Column 'NonNullable T.PGBool)
...
(.->) :: (PGIsJson a, PGJsonIndex k)
      => Column 'Nullable a -- ^
      -> Column 'NonNullable k -- ^ key or index
      -> Column 'Nullable a
(.->) = C.binOp (HPQ.:->)
...
(.#>) :: (PGIsJson a)
      => Column 'Nullable a -- ^
      -> Column 'NonNullable (T.PGArray 'NonNullable T.PGText) -- ^ path
      -> Column 'Nullable a
(.#>) = C.binOp (HPQ.:#>)
...
infix 4 .@>
(.@>) :: Column 'NonNullable T.PGJsonb -> Column 'NonNullable T.PGJsonb -> Column 'NonNullable T.PGBool
(.@>) = C.binOp (HPQ.:@>)
...
infix 4 .?
(.?) :: Column 'NonNullable T.PGJsonb -> Column 'NonNullable T.PGText -> Column 'NonNullable T.PGBool
(.?) = C.binOp (HPQ.:?)

-- | Do any of these key/element strings exist?
infix 4 .?|
(.?|) :: Column 'NonNullable T.PGJsonb
      -> Column 'NonNullable (T.PGArray 'NonNullable T.PGText) -> Column 'NonNullable T.PGBool
(.?|) = C.binOp (HPQ.:?|)
...
emptyArray :: T.IsSqlType a => Column 'NonNullable (T.PGArray n a)
emptyArray = T.pgArray id []

arrayPrepend :: Column n a -> Column 'NonNullable (T.PGArray n a) -> Column 'NonNullable (T.PGArray n a)
arrayPrepend (Column e) (Column es) = Column (HPQ.FunExpr "array_prepend" [e, es])

singletonArray :: T.IsSqlType a => Column n a -> Column 'NonNullable (T.PGArray n a)
singletonArray x = arrayPrepend x emptyArray

index :: (C.PGIntegral i) => Column 'NonNullable (T.PGArray 'NonNullable a) -> Column 'NonNullable i -> Column 'Nullable a
index (Column a) (Column b) = Column (HPQ.ArrayIndex a b)

-- * Other operators

timestamptzAtTimeZone :: Column 'NonNullable T.PGTimestamptz
                      -> Column 'NonNullable T.PGText
                      -> Column 'NonNullable T.PGTimestamp
timestamptzAtTimeZone = C.binOp HPQ.OpAtTimeZone

-- | Do not use.  Will be deprecated in version 0.6.  Use
-- 'C.unsafeCast' instead.
doubleOfInt :: Column 'NonNullable T.PGInt4 -> Column 'NonNullable T.PGFloat8
doubleOfInt (Column e) = Column (HPQ.CastExpr "float8" e)

overlap :: Column 'NonNullable (T.PGRange a) -> Column 'NonNullable (T.PGRange a) -> Column 'NonNullable T.PGBool
overlap = C.binOp (HPQ.:&&)

infix 4 .<<
(.<<) :: Column 'NonNullable (T.PGRange a) -> Column 'NonNullable (T.PGRange a) -> Column 'NonNullable T.PGBool
(.<<) = C.binOp (HPQ.:<<)
