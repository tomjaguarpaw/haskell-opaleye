...
representing it as a naked `Int32` we wrap it in a `WarehouseId` newtype
to guard against meaninglessly mixing it with other `Int32`s.  We can do
...
(Column PGInt4)` becomes a `WarehouseId' Int32` when the query is run.
We could run the query `queryTable goodWarehouseTable` like this.

> type WarehouseId = WarehouseId' Int32
> type GoodWarehouse = Warehouse' WarehouseId String Int32
>
