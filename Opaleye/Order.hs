...
desc :: (a -> C.Column b) -> OrderSpec a
desc = orderSpec PQ.OpDesc

asc :: (a -> C.Column b) -> OrderSpec a
asc = orderSpec PQ.OpAsc

orderBy :: OrderSpec a -> Query a -> Query a
orderBy os q =
  Q.simpleQueryArr (orderByU os . Q.runSimpleQueryArr q)
